...
Class Block_13_View extends BlockViewAbstract
{
    ...
    {
        $first_block = $this->render_block_type_1($results[0], 'jnews-360x504');

        $second_block = $third_block = '';
        for($i = 1; $i < sizeof($results); $i++) {
            if ( $i <= 2 )
            {
                $second_block .= $this->render_block_type_2($results[$i], 'jnews-350x250');
            } else {
                $third_block .= $this->render_block_type_2($results[$i], 'jnews-350x250');
            }
        }
    ...
    public function build_column_3($results)
    {
        ...
        for($i = 1; $i < sizeof($results); $i++) {
            if ( $i <= 2 ) {
                ...
                $fourth_block .= $this->render_block_type_2($results[$i], 'jnews-350x250');
            }
        }
