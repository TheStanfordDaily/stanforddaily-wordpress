...
Class Repeater extends ControlAbstract {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			$this->button_label = esc_attr__( 'Add new', 'jnews' ) . ' ' . $this->row_label['value'];
		}
			...
			if ( isset( $value['type'] ) && ( 'dropdown-pages' === $value['type'] ) ) {
				...
				$dropdown = wp_dropdown_pages(
					array(
						'name'              => '',
						'echo'              => 0,
						'show_option_none'  => esc_attr__( 'Select a Page', 'jnews' ),
						'option_none_value' => '0',
						'selected'          => '',
					)
				);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'jnews' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
