...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		else throw new Exception( 'Option Key is required', 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception( 'Template Array/File is required', 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception( 'Page Slug is required', 1);
	...
	public function register_menu_page()
	{
		...
		$this->set_hook_suffix( get_plugin_page_hookname( $this->get_page_slug(), '' ) );
	...
	{
		VP_WP_Util::admin_notice("Development Mode is Active, options' values won't be saved into database.", false);
	}
	...
	{
		do_action('vp_before_render_set');
		echo vp_sanitize_output($this->get_options_set()->render());
		do_action('vp_after_render_set');
	}
	...
	{
		$this->_option_key = apply_filters('vp_option_key', $_option_key);
		return $this;
	}
