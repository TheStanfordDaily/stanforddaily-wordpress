...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = '[Development Mode] ' . $this->title;
		}
	...
	function _render_view($fields)
	{
		if(!empty($this->tab_header)) {
			echo "<div class='vp-tabs'>";
			$this->_render_tab_menu();
			$this->_enview($fields);
			echo "</div>";
		} else {
			$this->_enview($fields);
	...
	function _render_tab_menu()
	{
		$html = '<ul>';
		foreach($this->tab_header as $header) {
			$html .=  "<li><a href=\"#{$header['name']}\">{$header['title']}</a></li>";
		}
		$html .= '</ul>';
		echo vp_sanitize_output($html);
	}
	...
	function _enfactor($arr)
	{
		...
		foreach ($fields as $field)
		{
			...
			{
				$field_objects[$field['name']] = $this->_enfactor_tab($field, $mb);
			}
			else
			{
		...
		if(!function_exists('loop_fields'))
		{
			function loop_fields(&$fields)
			{
				foreach ($fields as &$field)
				{
					...
					if ( $field instanceof VP_Control_Field )
					{
						$active_callback = $field->get_active_callback();
	...
	function _enfactor_tab($field, $mb)
	{
	...
	function _enview($fields)
	{
		foreach ($fields as $name => $field)
		{
			if( is_array($field) and isset($field['tabs']) )
			{
				echo vp_sanitize_output($this->_render_tab($field));
			}
			else if( is_array($field) and $field['repeating'] )
			{
				echo vp_sanitize_output($this->_render_repeating_group($field));
			}
			else if( is_array($field) and !$field['repeating'] )
			{
				echo vp_sanitize_output($this->_render_group($field));
			}
			else
			{
				echo vp_sanitize_output($this->_render_field($field));
			}
		}
	}
	...
	function _render_tab($tab)
	{
		$html = '';
		$html .= "<div id='{$tab['name']}'>";

		foreach($tab['tabs'] as $f)
		{
	...
	function _render_group($group)
	{
		...
		$active_callback = isset( $group['active_callback'] ) ? 'data-active-callback="' . esc_attr( json_encode( $group['active_callback'] ) ) . '"' : '';
		...
		$html .= '<div id="wpa_loop-' . $uid
				...
				. (empty($active_callback) ? '' : ' vp-hide vp-dep-inactive')
				. '"'
	...
	function _render_repeating_group($group)
	{
        ...
        $groupsize = sizeof($group['groups']);
		foreach ($group['groups'] as $index => $g)
		{

			// set title of group (only if section is setted)

            $grouptitle = $group['title'];
            foreach ($g['childs'] as $f)
            {
                $value = $f->get_value();
                if($f->is_attach_title() && !empty($value) && $groupsize > ($index + 1)) {
                    $grouptitle .= " - " . $f->get_value();
                }
            }

            $grouptitle = apply_filters('jeg_modify_repeating_title', $grouptitle , $g);
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $grouptitle . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. 'Remove' .'"><i class="icon-remove"></i> '. 'Remove' .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		// $html .= '<a href="#" class="button button-large button-primary docopy-' . $name . '">'. 'Add More' . ' ' . $group['title'] . '</a>';
		$html .= '<a href="#" class="button button-large button-primary docopy-' . $name . '"><i class="fa fa-plus"></i></a>';
		$html .= '</div>';
