...
class VP_ShortcodeGenerator
{
	...
	public function can_output()
	{
        ...
        if(!empty($this->include_page_template) && $screen === 'page')
        {
            $templatefile = jeg_get_current_page_template_name();

            if(in_array($templatefile, $this->include_page_template))
            {
                $can &= true;
            } else
            {
        ...
        $can &= apply_filters($this->name . '_can_render_shortcode', $can, $screen);
			...
			<div class="vp-sc-scroll-container">
				<div class="vp-sc-wrapper">
					...
					<?php foreach ($this->template as $title => $menu): ?>
						...
						<li class="current"><a href="#<?php echo str_replace(' ', '_', $title); ?>"><?php echo esc_html($title); ?></li></a>
						<?php else: ?>
						<li><a href="#<?php echo str_replace(' ', '_', $title); ?>"><?php echo esc_html($title); ?></li></a>
						<?php endif; ?>
					<?php endforeach; ?>
					</ul>
								...
								<li class="vp-sc-element postbox<?php if(isset($element['attributes'])) echo ' has-options'; ?>">
									<h3 class="hndle vp-sc-element-heading">
										<a href="#">
											...
											<?php if(isset($element['attributes'])) echo '<i class="fa fa-caret-square-o-down"></i>'; ?>
										</a>
									</h3>
									...
									<form class="vp-sc-element-form vp-hide inside">
										<?php echo vp_sanitize_output($this->print_form($element['attributes'])); ?>
									</form>
									<?php endif; ?>
								</li>
							<?php endforeach; ?>
	...
	function print_form($attributes)
	{
		...
		foreach ($attributes as $attr)
		{
			...
			<?php if($attr['type'] !== 'notebox'): ?>
				<div class="vp-sc-field vp-<?php echo esc_attr($attr['type']); ?>" data-vp-type="vp-<?php echo esc_attr($attr['type']); ?>">
					<div class="label"><label><?php echo esc_html($attr['label']); ?></label></div>
					<div class="field"><div class="input"><?php echo esc_html($field->render(true)); ?></div></div>
				</div>
			<?php else: ?>
				...
				<div class="vp-sc-field vp-<?php echo esc_attr($attr['type']); ?> note-<?php echo esc_attr($status); ?>" data-vp-type="vp-<?php echo esc_attr($attr['type']); ?>">
					<?php echo vp_sanitize_output($field->render(true)); ?>
				</div>
			<?php endif; ?>
		...
		<div class="vp-sc-action">
			<button class="vp-sc-insert button"><?php echo 'insert'; ?></button>
			<button class="vp-sc-cancel button"><?php echo 'cancel'; ?></button>
		</div>
		<?php
	}
