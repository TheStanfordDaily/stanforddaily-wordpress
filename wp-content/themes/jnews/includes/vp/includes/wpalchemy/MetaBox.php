...
class WPAlchemy_MetaBox
{
	...
	function _import($post_id, $key, $value)
	{
		if (WPALCHEMY_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
            ...
            if($wp_import && is_array($wp_import->posts))
            {
	...
	function _head()
	{
		...
		echo vp_sanitize_output($content);
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
        ...
        // Fix issue with Calendarize plugin
		global $post;
		...
		if ( $rhc_template_id > 0 )
		{
			$meta = get_post_meta($rhc_template_id, $this->id, TRUE);
		}
	...
	function the_id()
	{
		echo vp_sanitize_output($this->get_the_id());
	}
	...
	{
		echo vp_sanitize_output($this->get_the_value($n));
	}
	...
	{
		echo vp_sanitize_output($this->get_the_name($n));
	}
	...
	function the_index()
	{
		echo vp_sanitize_output($this->get_the_index());
	}
	...
	{
		echo vp_sanitize_output($this->get_the_checkbox_state($n, $v));
	}
	...
	{
		echo vp_sanitize_output($this->get_the_select_state($n, $v));
	}
	...
	{
		echo vp_sanitize_output($this->get_the_group_open($t));
	}
	...
	{
		echo vp_sanitize_output($this->get_the_group_close());
	}
...
class WPA_Loop
{
	...
	{
		echo vp_sanitize_output($this->get_the_indexed_name());
	}
