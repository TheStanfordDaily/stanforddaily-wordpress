<div class="">
	<div id="vp-wrap" class="vp-wrap">
		<div id="vp-option-panel"class="vp-option-panel <?php echo ($set->get_layout() === 'fixed') ? 'fixed-layout' : 'fluid-layout' ; ?>">
			<div class="vp-left-panel">
				<div id="vp-logo" class="vp-logo">
					<h2>
						<?php echo esc_html($set->get_title()); ?>
						<?php echo 'v' . esc_html($set->get_version()); ?>
					</h2>
				</div>
			...
			<div class="vp-right-panel">
				<form id="vp-option-form" class="vp-option-form vp-js-option-form" method="POST">
					<div id="vp-submit-top" class="vp-submit top">
						<div class="inner">
							<input class="vp-save vp-button button button-primary" type="submit" value="<?php echo 'Save Changes'; ?>" />
							<p class="vp-js-save-loader save-loader" style="display: none;"><img src="<?php VP_Util_Res::img_out('ajax-loader.gif', ''); ?>" /><?php echo 'Saving Now'; ?></p>
					...
					<?php if ($menu === reset($menus)): ?>
						<?php echo vp_sanitize_output($menu->render(array('current' => 1))); ?>
					<?php else: ?>
						<?php echo vp_sanitize_output($menu->render(array('current' => 0))); ?>
					<?php endif; ?>
					<?php endforeach; ?>
		...
		<div id="vp-copyright" class="vp-copyright">
			<p><?php printf('This option panel is built using Vafpress Framework %s', VP_VERSION); ?></p>
		</div>
	</div>
</div>